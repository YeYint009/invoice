...
const createListUi = (text) => {
  ...
  list.innerHTML = ` 
  <div class="animate__animated animate__fadeInUp flex justify-between mb-5 border-2 border-black p-5">
    <div class="text-box flex items-center">
      <input type="checkbox" class="w-5 h-5" name="" id="" />
      <div class="tasklist-text ms-2">${text}</div>
    </div>
    <div class="">
  ...
  //checking
  const textListCheckBox = list.querySelector("[type ='checkbox']");
  textListCheckBox.addEventListener("click", listCheckHandler);
  return list;
};
